...
export default async function PaperPage({
  ...
  const prev = workingPapers[(index + workingPapers.length - 1) % workingPapers.length];
    ...
    >
      ...
      <div style={{ background: meta.card, color: meta.cardInk }}>
        <div className="mx-auto flex max-w-3xl flex-wrap items-center justify-between gap-3 px-6 py-3">
          <Link href="/lab/bento/research" className="bn-label hover:opacity-70">
            ← All research
          </Link>
        ...
        {sections.map((section, i) => (
          <Reveal key={section.key}>
            <section className="mt-12">
              ...
              <div
                className={`rd-prose mt-5 ${i === 0 ? "rd-dropcap" : ""}`}
              >
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              {[
                ...
                return (
                  <Link
                    key={label}
                    href={`/lab/bento/research/${p.slug}`}
